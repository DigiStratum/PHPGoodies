<?php
/**
 * PHPGoodies:MathBackgroundAttribute - MATHBACKGROUND element attribute trait for NodeElements to easily use
 *
 * @author Sean M. Kelly <smk@smkelly.com>
 */

/**
 * MathBackgroundAttribute - MATHBACKGROUND element attribute trait for NodeElements to easily use
 */
trait MathBackgroundAttribute {
	/**
	 * Set the mathbackground attribute value
	 *
	 * @param string $value The value to set for this attribute
	 *
	 * @return object This object for chaining...
	 */
	public function setMathBackground($value) {
		$this->setAttribute('mathbackground', $value);

		return $this;
	}

	/**
	 * Get the mathbackground attribute's current value
	 *
	 * @return string The attribute's current value or null if not set
	 */
	public function getMathBackground() {
		return $this->getAttribute('mathbackground');
	}
}

